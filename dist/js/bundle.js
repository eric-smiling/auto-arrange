...
function euclideanSort(rect, getter) {
  ...
  return function (a, b) {
    var aX = getter ? getter(a) : a;
    var bX = getter ? getter(b) : b;

    /* eslint-disable no-restricted-properties */
    var aDist = Math.sqrt(Math.pow(aX.left + aX.width / 2 - (rect.left + rect.width / 2), 2) + Math.pow(aX.top + aX.height / 2 - (rect.top + rect.height / 2), 2));

    var bDist = Math.sqrt(Math.pow(bX.left + bX.width / 2 - (rect.left + rect.width / 2), 2) + Math.pow(bX.top + bX.height / 2 - (rect.top + rect.height / 2), 2));
    /* eslint-disable no-restricted-properties */
...
exports.clone = clone;
exports.euclideanSort = euclideanSort;
exports.testIntersection = testIntersection;
exports.testIntersections = testIntersections;
...
/* global document */
var NODE_QUERY_SELECTOR = '.draggable';

// primary method for obtaining draggable nodes from the dom
var getNodes = function getNodes() {
  return Array.from(document.querySelectorAll(NODE_QUERY_SELECTOR));
};

// create context for auto-arranging nodes
var context = new _Context2.default(getNodes);

// timer used for event handling while dragging
var timer = void 0;

// target elements with the "draggable" class
(0, _interactjs2.default)(NODE_QUERY_SELECTOR).draggable({
  ...
  onmove: function onmove(event) {
    ...
    // update the context
    var targetPos = context.getPositionForNode(target);
...
getNodes().forEach(function (node) {
  (0, _interactjs2.default)(node).on('tap', function (e) {
    return context.doToggleSize(e.target);
  }, true).on('mousedown', function (e) {
    return e.target.classList.remove('repelling');
  }, true);
});
...
var Context = function () {
  ...
  _createClass(Context, [{
    ...
    value: function start() {
      ...
      this.positions.forEach(function (v, k) {
        return _this.initialPositions.set(k, (0, _utils.clone)(v));
      });
    }
  }, {
    key: 'move',
    ...
    key: 'discoverPositions',
    value: function discoverPositions() {
      var _this2 = this;

      this.getNodes().forEach(function (node) {
        _this2.positions.set(node, {
          id: node.id,
          rect: (0, _utils.clone)(node.getBoundingClientRect())
        });
      });
    }
  }, {
    key: 'doRevert',
    value: function doRevert(target) {
      (0, _actions.revert)(target, this.positions, this.initialPositions, this.movedNodes, this.getNodes);
    }
  }, {
    key: 'doRepel',
    value: function doRepel(target, ignore) {
      (0, _actions.repel)(target, ignore, this.positions, this.causalNodes, this.movedNodes, this.getNodes);
    }
  }, {
    key: 'doToggleSize',
    value: function doToggleSize(target) {
      (0, _actions.toggleSize)(target, this.positions, this.initialPositions, this.causalNodes, this.movedNodes, this.attractedNodes, this.getNodes.bind(this), this.discoverPositions.bind(this));
    }
  }, {
    key: 'getPositions',
    ...
    key: 'getPositionForNode',
    value: function getPositionForNode(node) {
      return this.positions.get(node);
    }
  }, {
    key: 'setPositionForNode',
    value: function setPositionForNode(node, position) {
      this.positions.set(node, position);
    }
  }, {
    key: 'clearMovedNodes',
    value: function clearMovedNodes() {
      this.movedNodes.clear();
    }
  }, {
    key: 'clearInitialPositions',
    value: function clearInitialPositions() {
      this.initialPositions.clear();
    }
  }, {
    key: 'clearCausalNodes',
    value: function clearCausalNodes() {
...
exports.toggleSize = exports.revert = exports.repel = undefined;
...
function attract(target, dx, dy, ignore, pos, getNodes, attd) {
  ...
  var tp = pos.get(target);
  var nodes = getNodes();

  nodes.sort((0, _utils.euclideanSort)(tp.rect, function (x) {
    return pos.get(x).rect;
  }));
  ...
  nodes.forEach(function (node) {
    ...
    var mx = ep.rect.left - tp.rect.right + dx;
    if (mx >= 0 && mx <= _constants.MARGIN * 2 && (ep.rect.top < tp.rect.bottom - dx && ep.rect.bottom > tp.rect.top - dx || ep.rect.bottom > tp.rect.top - dx && ep.rect.top < tp.rect.bottom - dx)) {
      // move left
      var cp = {
        id: node.id,
        rect: {
      ...
      if (!(0, _utils.testIntersections)(cp, nodes.map(function (e) {
        return pos.get(e);
      }))) {
        ep.rect.left += dx;
        ep.rect.right += dx;
    ...
    var my = ep.rect.top - tp.rect.bottom + dy;
    if (my >= 0 && my <= _constants.MARGIN * 2 && (ep.rect.left < tp.rect.right && ep.rect.right > tp.rect.left || ep.rect.right > tp.rect.left && ep.rect.left < tp.rect.right)) {
      // move up
      var _cp = {
        id: node.id,
        rect: {
      ...
      if (!(0, _utils.testIntersections)(_cp, nodes.map(function (e) {
        return pos.get(e);
      }))) {
        ep.rect.top += dy;
        ep.rect.bottom += dy;
...
function repel(target, ignore, pos, cause, moved, getNodes) {
  ...
  getNodes().forEach(function (node) {
    ...
    if ((0, _utils.testIntersection)(tp, ep)) {
      ...
      var ox1 = Math.max(tp.rect.left, ep.rect.left);
      var ox2 = Math.min(tp.rect.right, ep.rect.right);

      var xSign = (ep.rect.right + ep.rect.left) / 2 > (tp.rect.right + tp.rect.left) / 2 ? 1 : -1;
      var xDisplacement = xSign * (ox2 - ox1);

      var oy1 = Math.max(tp.rect.top, ep.rect.top);
      var oy2 = Math.min(tp.rect.bottom, ep.rect.bottom);

      var ySign = (ep.rect.bottom + ep.rect.top) / 2 > (tp.rect.bottom + tp.rect.top) / 2 ? 1 : -1;
      var yDisplacement = ySign * (oy2 - oy1);
      ...
      if (Math.abs(xDisplacement) < Math.abs(yDisplacement)) {
        xDisplacement = xDisplacement > 0 ? Math.ceil(xDisplacement) + _constants.MARGIN : Math.floor(xDisplacement) - _constants.MARGIN;
        style.left = ex.rect.left + xDisplacement + 'px';
        ex.rect.left = parseInt(style.left, 10);
        ex.rect.right = ex.rect.left + ex.rect.width;
      } else {
        yDisplacement = yDisplacement > 0 ? Math.ceil(yDisplacement) + _constants.MARGIN : Math.floor(yDisplacement) - _constants.MARGIN;
...
function revert(target, pos, init, moved, getNodes) {
  ...
  nodes.forEach(function (el) {
    ...
    if (moved.has(el) && !(0, _utils.testIntersection)(initRect, pos.get(target))) {
      var inter = (0, _utils.testIntersections)(init.get(el), nodes.map(function (cel) {
        var p = pos.get(cel);
        p.id = cel.id;
        return p;
      }));
...
function toggleSize(el, pos, init, cause, moved, attd, getNodes, discoverPositions) {
  ...
  if (!el.classList.contains('collapsed')) {
    var _pos$get$rect = pos.get(el).rect,
        w0 = _pos$get$rect.width,
        h0 = _pos$get$rect.height;

    el.classList.add('collapsed');
    discoverPositions();
    var _pos$get$rect2 = pos.get(el).rect,
        w1 = _pos$get$rect2.width,
        h1 = _pos$get$rect2.height;
    ...
    pos.forEach(function (v, k) {
      return init.set(k, (0, _utils.clone)(v));
    });
